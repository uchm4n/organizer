    ...
    ->group(function (): void {
        ...
        Route::middleware('auth:sanctum')->group(function (): void {
            ...
            Route::middleware('role:admin')->group(function (): void {
                ...
                Route::post('/workspaces', WorkspaceGeneralStoreController::class)->name('workspace.general.store');
            });

            // Workspace — own (authenticated users)
            Route::get('/workspace', WorkspaceShowController::class)->name('workspace.show');
            Route::post('/workspace', WorkspaceStoreController::class)->name('workspace.store');
            Route::patch('/workspace', WorkspaceUpdateController::class)->name('workspace.update');

            // Workspace — admin or owner
            Route::get('/workspaces/{workspace}', WorkspaceGeneralShowController::class)->name('workspace.general.show');
            Route::patch('/workspaces/{workspace}', WorkspaceGeneralUpdateController::class)->name('workspace.general.update');
            Route::delete('/workspaces/{workspace}', WorkspaceGeneralDestroyController::class)->name('workspace.general.destroy');
            ...
            Route::post('/items', ItemStoreController::class)->name('item.store');
            Route::get('/items/{item}', ItemShowController::class)->name('item.show');
            Route::patch('/items/{item}', ItemUpdateController::class)->name('item.update');
            Route::delete('/items/{item}', ItemDestroyController::class)->name('item.destroy');
            Route::post('/items/{id}/restore', ItemRestoreController::class)->name('item.restore');
        });
    });
